...
pub enum CombatState {
    GettingAction(GettingAction),
    Waiting(Waiting),
}

impl CombatState {
    pub fn new(combatants: Vec<Combatant>) -> Self {
        CombatState::Waiting(Waiting { combatants })
    }

    pub fn transform(mut self, event: Event) -> State {
        let state = match self {
            CombatState::GettingAction(state) => CombatState::GettingAction(state),
            CombatState::Waiting(state) => CombatState::Waiting(state),
        };
...
//        turn_order.sort_by(|a, b| combatants[*b].attribute(Stat::Agility).cmp(&combatants[*a].attribute(Stat::Agility)));
//
//        Self {
